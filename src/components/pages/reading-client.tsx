...
// Add coverUrl to schema
const bookSchema = z.object({
  title: z.string().min(1, "Title is required"),
  author: z.string().min(1, "Author is required"),
  progress: z.coerce.number().min(0).max(100),
  coverUrl: z.string().url("Must be a valid image URL").optional(),
});

type Book = z.infer<typeof bookSchema> & { id: number };

// Example starter books with covers
...
export default function ReadingClient() {
  ...
  const form = useForm<z.infer<typeof bookSchema>>({
    ...
    defaultValues: { title: "", author: "", progress: 0, coverUrl: "" },
  });

  const getRecommendationsForm = useForm({
    defaultValues: {
      readingHistory: initialBooks
  ...
  function addBook(values: z.infer<typeof bookSchema>) {
    ...
    toast({
      title: "Book Added",
      description: `"${values.title}" has been added to your log.`,
    });
  }

  async function handleGetRecommendations(values: {
      ...
      <div className="lg:col-span-2 space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Your current and completed reads.
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Sparkles className="text-accent h-5 w-5" /> AI Reading
              Recommendations
            </CardTitle>
            <CardDescription>
          ...
          <Form {...getRecommendationsForm}>
            <form
              onSubmit={getRecommendationsForm.handleSubmit(
                handleGetRecommendations
              )}
            >
              <CardContent className="space-y-4">
                <FormField
                  control={getRecommendationsForm.control}
                  name="readingHistory"
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea
                          placeholder="Tell us what you've read..."
                          {...field}
                          rows={4}
                        />
                      </FormControl>
